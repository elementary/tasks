...
public class Tasks.LocationPopover : Tasks.EntryPopover<Geocode.Location?> {
    ...
    private GtkChamplain.Embed map_embed = new GtkChamplain.Embed () {
        height_request = 120,
        width_request = 220
    };
    ...
    private Marker point = new Marker () {
        draggable = true
    };
    ...
    private void on_search_entry_activate () {
        ...
        value = new Geocode.Location.with_description (
            0,
            0,
            (value == null ? Geocode.LocationAccuracy.UNKNOWN : value.accuracy),
            search_entry.text
        );
    }
    ...
    private void on_location_mode_changed () {
        ...
        var accuracy = (value == null ? Geocode.LocationAccuracy.UNKNOWN : value.accuracy).abs ();
        if (location_mode.selected == 1) {
            accuracy = accuracy * -1;
        }

        value = new Geocode.Location.with_description (
            (value == null ? 0 : value.longitude),
            (value == null ? 0 : value.latitude),
            accuracy,
            search_entry.text
        );
    }
