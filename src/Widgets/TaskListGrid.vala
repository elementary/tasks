...
public class Tasks.Widgets.TaskListGrid : Gtk.Grid {
    ...
    private EditableLabel editable_title_label;
    ...
    construct {
        editable_title_label = new EditableLabel () {
            margin_start = 24
        };

        unowned Gtk.StyleContext title_label_style_context = editable_title_label.get_style_context ();
        ...
        column_spacing = 12;
        attach (editable_title_label, 0, 0);
        ...
        editable_title_label.changed.connect (() => {
            Application.model.update_task_list_display_name.begin (source, editable_title_label.text, (obj, res) => {
                GLib.Idle.add (() => {
                    try {
                        Application.model.update_task_list_display_name.end (res);
                    } catch (Error e) {
                        editable_title_label.text = source.display_name;
    ...
    public void update_request () {
        editable_title_label.text = source.dup_display_name ();

        Tasks.Application.set_task_color (source, editable_title_label);
